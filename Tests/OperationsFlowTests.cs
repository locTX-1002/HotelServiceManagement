...
public class OperationsFlowTests
{
    ...
    public async Task PhanQuyenXuLyDon_NhanVienDichVuManagerAdminDuoc_LeTanBiChan()
    {
        ...
        Assert.False(byAdmin.Ok, "Admin không được xử lý đơn dịch vụ.");
    }
    ...
    public async Task PhanQuyenBaoCao_ChiManagerXemDuoc()
    {
        ...
        var revenueOk = await reports.GetRevenueAsync(today.AddDays(-7), today);
        Assert.True(revenueOk.Ok, revenueOk.Message);

        var occupancyOk = await reports.GetOccupancyAsync();
        Assert.True(occupancyOk.Ok, occupancyOk.Message);
    }
}
