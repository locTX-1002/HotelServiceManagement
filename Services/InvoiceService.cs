...
public sealed class InvoiceService : IInvoiceService
{
    ...
    public async Task<ServiceResult<Invoice>> PrepareAsync(int stayId, string? promotionCode = null, DateTime? asOf = null)
    {
        ...
        var invoice = stay.Invoice; var isNew = invoice == null; var paid = invoice?.Payments.Where(p => p.Status == PaymentStatus.Completed).Sum(p => p.Amount) ?? 0; if (paid > 0) return ServiceResult<Invoice>.Failure("Hoá đơn đã có thanh toán nên không tính lại được.");
