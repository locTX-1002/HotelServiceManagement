...
public sealed class ReportDAO
{
    ...
    public async Task<List<Invoice>> GetInvoicesAsync(DateTime from, DateTime toExclusive) { await using var c = HotelDbContextFactory.Create(); return await c.Invoices.AsNoTracking().Include(x => x.Stay).ThenInclude(s => s.Reservation).ThenInclude(r => r.Room).Where(x => x.Status != InvoiceStatus.Cancelled && x.InvoiceDate >= from && x.InvoiceDate < toExclusive).ToListAsync(); }
