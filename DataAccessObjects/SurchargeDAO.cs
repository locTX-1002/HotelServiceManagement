...
public sealed class SurchargeDAO
{
    ...
    /// <summary>Tong phu thu theo tung luot luu tru - hien nhanh tren danh sach, khoi goi lap.</summary>
    public async Task<Dictionary<int, decimal>> GetTotalsAsync(IEnumerable<int> stayIds)
    {
        var ids = stayIds.Distinct().ToList();
        if (ids.Count == 0) return [];
        await using var c = HotelDbContextFactory.Create();
        return await c.Surcharges.AsNoTracking()
            .Where(x => ids.Contains(x.StayId))
            .GroupBy(x => x.StayId)
