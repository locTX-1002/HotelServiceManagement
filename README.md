...
> **Mới vào dự án?** Đọc [Hướng dẫn chạy](docs/HUONG_DAN_CHAY.md) — 5 phút là
> chạy được, không cần ai gửi file database.
...
4. Máy bạn không dùng `.\SQLEXPRESS`? → **đừng sửa file chung** — tạo file
   ...
   ```json
   {
     ...
     "BootstrapAdmin": {
       ...
       "Password": "MatKhauManhCuaBan@2026"
     }
   }
   ```

Tài khoản Admin được bootstrap từ file local, mật khẩu được hash BCrypt trước khi lưu database.
Các tài khoản demo mặc định bị khóa; Admin tạo tài khoản nhân viên thật trong chức năng quản lý nhân viên.
...
- Không cần chạy `database update` tay — app tự `MigrateAsync()` lúc khởi động.
  Nếu vẫn muốn chạy tay thì phải để `DataAccessObjects` làm cả startup project:

  ```bash
  dotnet ef database update --project DataAccessObjects
  ```

  Chỉ định `--startup-project FUHotelManagementWPF` sẽ **báo lỗi**
  `doesn't reference Microsoft.EntityFrameworkCore.Design` — đúng như thiết kế,
  vì tầng WPF cố tình không tham chiếu gói EF nào. `DataAccessObjects` có sẵn
  `DesignTimeDbContextFactory` nên tự đứng một mình được.
- Trước khi chạy lệnh EF nào cũng nên **tắt app đang chạy**, không thì build
  hỏng vì file DLL đang bị khoá.
...
Chạy bộ kiểm thử backend trước khi mở PR:
